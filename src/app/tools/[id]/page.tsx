...
// 🔐 Browser Fingerprint (Manual - لا يحتاج npm install)
async function generateFingerprint(): Promise<string> {
  ...
  return Math.abs(hash).toString(16) + btoa(str.slice(0, 40)).replace(/[^a-zA-Z0-9]/g, '').slice(0, 16);
}
...
export default function ToolDetailPage() {
  ...
  useEffect(() => {
    if (id) {
      fetchToolAndReviews();
      // توليد البصمة والتحقق من التقييم السابق
